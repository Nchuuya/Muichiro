...
async def inline_query_handler(client, query):
            ...
            results=[
                InlineQueryResultPhoto(
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        ...
                        [
                        InlineKeyboardButton(text="Harem", switch_inline_query_current_chat="harem ")
                        ]]
                    )
                ),
                InlineQueryResultPhoto(
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        [[
                        InlineKeyboardButton(text="Support", url="https://t.me/BoaHancock_support"),
                        InlineKeyboardButton(text="Channel", url="https://t.me/boa_updates")                   
                        ],
                        [
    ...
    elif string.split()[0] == "harem":
        user = query.from_user
        fvrt_char = list(REDIS.sunion(f"anime_waifu{user.id}"))
        for x in fvrt_char:
            search = x
            variabes = {'query': search}
            kk = requests.post(url, json={
                                 'query': character_query, 'variables': variabes}).json()['data']['Character']
            if kk:
                image = kk.get('image', None)
                if fvrt_char:
                    image = image.get('large')
                    limit = 0
                    for i in fvrt_char:
                        if limit > 48:
                            break
                        limit += 1
                        harem_txt = "{}'s Waifu In Their Harem:\n • {}".format(user.username, i)
                        variables = {'query': i}
                        json = requests.post(url, json={
                                             'query': character_query, 'variables': variables}).json()['data']['Character']
                        answers.append(InlineQueryResultPhoto(
                            caption=harem_txt,
                            photo_url=json.get('image', None).get('large'),
                            parse_mode="html",
                            title="check your waifus",
                            description=f"click here to see your waifus"))
                else:
                    answers.append(InlineQueryResultArticle(
                        title=f"check your waifus",
                        description=f"click here to see your waifus",
                        input_message_content=InputTextMessageContent(
                            "You Don't Have Waifus", parse_mode="markdown", disable_web_page_preview=True)))
                await client.answer_inline_query(query.id,
                                             results=answers,
                                             cache_time=0,
                                             is_gallery=True
